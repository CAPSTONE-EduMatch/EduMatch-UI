...
async function cleanDatabase() {
	...
	try {
		...
		await prismaClient.stripePayment.deleteMany({});
		...
		await prismaClient.userManagement.deleteMany({});
		await prismaClient.postManagement.deleteMany({});
		await prismaClient.planManagement.deleteMany({});
		...
		await prismaClient.user.deleteMany({}); // Delete users
		...
		await prismaClient.systemManager.deleteMany({});
	...
	const plans = [
		{
			...
			month_price: 999, // $9.99 in cents
			year_price: 9999, // $99.99 in cents
			status: true,
			create_at: new Date(),
			type: true,
		},
		{
			...
			month_price: 1999, // $19.99 in cents
		...
		},
		{
			...
			month_price: 9999, // $99.99 in cents
			year_price: 99999, // $999.99 in cents
			status: true,
			create_at: new Date(),
			type: false,
		},
...
async function seedDisciplines() {
	...
	const disciplines = fields.map((field, index) => ({
		discipline_id: `550e8400-e29b-41d4-a716-44665544${String(index + 1).padStart(4, "0")}`,
		name: field,
		status: true,
	}));
...
async function seedSubdisciplines() {
	...
	for (let i = 0; i < fields.length; i++) {
		...
		for (let j = 0; j < numSubdisciplines; j++) {
			subdisciplines.push({
				subdiscipline_id: `550e8400-e29b-41d4-a716-44665544${String(subdisciplineId).padStart(4, "0")}`,
				discipline_id: `550e8400-e29b-41d4-a716-44665544${String(i + 1).padStart(4, "0")}`,
...
async function seedApplicants() {
	...
	for (let i = 1; i <= 10; i++) {
		applicants.push({
			applicant_id: `applicant-${i.toString().padStart(3, "0")}`,
			...
			subdiscipline_id: `550e8400-e29b-41d4-a716-44665544${String(Math.floor(Math.random() * 100) + 1).padStart(4, "0")}`,
	...
	console.log("✅ 10 Applicants seeded successfully");
}
...
async function seedInstitutions() {
	...
	for (let i = 1; i <= 10; i++) {
		...
		institutions.push({
			institution_id: `institution-${i.toString().padStart(3, "0")}`,
			user_id: `user-${(i + 10).toString().padStart(3, "0")}`, // Link to user account (users 11-20)
			name: university,
	...
	console.log("✅ 10 Institutions seeded successfully");
}
...
async function seedOpportunityPosts() {
	...
	for (let i = 1; i <= 200; i++) {
		...
		const university = getRandomElement(universities);
		const institutionId = `institution-${String(Math.floor(Math.random() * 10) + 1).padStart(3, "0")}`;

		posts.push({
			post_id: `550e8400-e29b-41d4-a716-44665547${String(i).padStart(4, "0")}`,
			title: `${university} ${field} Program`,
			other_info: generateProgramDescription(field, university),
			status: getRandomElement(["PUBLISHED", "DRAFT", "ARCHIVED"]) as any,
...
async function seedWishlists() {
	...
	for (let i = 1; i <= 100; i++) {
		...
		for (let j = 0; j < numWishlistItems; j++) {
			...
			do {
				applicantId = `550e8400-e29b-41d4-a716-44665545${String(Math.floor(Math.random() * 50) + 1).padStart(4, "0")}`;
...
async function seedApplications() {
	...
	for (let i = 1; i <= 5; i++) {
		applications.push({
			application_id: `550e8400-e29b-41d4-a716-44665548${String(i).padStart(4, "0")}`,
			post_id: `550e8400-e29b-41d4-a716-44665547${String(Math.floor(Math.random() * 200) + 1).padStart(4, "0")}`,
			applicant_id: `550e8400-e29b-41d4-a716-44665545${String(Math.floor(Math.random() * 50) + 1).padStart(4, "0")}`,
...
async function seedNotifications() {
	...
	for (let i = 1; i <= 100; i++) {
		notifications.push({
			notification_id: `550e8400-e29b-41d4-a716-44665549${String(i).padStart(4, "0")}`,
			user_id: `550e8400-e29b-41d4-a716-44665545${String(Math.floor(Math.random() * 50) + 1).padStart(4, "0")}`,
