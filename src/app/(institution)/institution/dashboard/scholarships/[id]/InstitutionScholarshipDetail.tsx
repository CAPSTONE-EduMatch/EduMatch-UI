...
const InstitutionScholarshipDetail = () => {
	...
	const fetchSuggestedApplicants = async (scholarshipId: string) => {
		try {
			setIsLoadingSuggested(true)
			const response = await fetch(
				`/api/applications/institution/suggested?postId=${scholarshipId}&minMatchScore=70`
			)
	...
	useEffect(() => {
		const loadScholarshipData = async () => {
			...
			if (scholarshipData) {
				...
				await fetchSuggestedApplicants(scholarshipId)
			}
		}
