...
export async function POST(request: NextRequest) {
	try {
		...
		const normalizedNewTitle = body.scholarshipName.trim();
...
export async function PUT(request: NextRequest) {
	try {
		...
		if (updateData.scholarshipName) {
			...
			// Check for case-insensitive match (trim and compare)
			const normalizedNewTitle = updateData.scholarshipName.trim();
			const duplicatePost = allPublishedPosts.find(
				(post) =>
					post.title.trim().toLowerCase() ===
					normalizedNewTitle.toLowerCase()
			...
			if (duplicatePost) {
				return NextResponse.json(
					{
						error: `A published post with the title "${updateData.scholarshipName}" already exists. Please use a different title.`,
					},
					{ status: 409 } // 409 Conflict
				);
			}
