...
export async function generatePresignedPost(bucketName: string, key: string, fileType: string = "image/jpeg") {
  try {
    if (!bucketName || !key) {
...
export async function listS3Objects(bucketName: string, prefix: string = "uploads/") {
  const s3Client = new S3Client({
    region: process.env.S3_REGION,
    credentials: {
      accessKeyId: process.env.S3_ACCESS_KEY_ID || "",
...
export async function generatePresignedGetUrl(bucketName: string, key: string, expiresIn: number = 3600) {
  const s3Client = new S3Client({
    region: process.env.S3_REGION,
    credentials: {
      accessKeyId: process.env.S3_ACCESS_KEY_ID || "",
