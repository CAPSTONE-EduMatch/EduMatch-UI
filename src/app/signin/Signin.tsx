...
const SignIn: React.FC = () => {
  ...
  const [errors, setErrors] = useState<{ email?: string; password?: string; forgotEmail?: string }>({});
  const [isLoading, setIsLoading] = useState(false);
  const [showForgotPassword, setShowForgotPassword] = useState(false);
  const [forgotPasswordEmail, setForgotPasswordEmail] = useState("");
  const [forgotPasswordStatus, setForgotPasswordStatus] = useState<{ success?: string; error?: string }>({});
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        setErrors(prev => ({ ...prev, forgotEmail: error.message || "Failed to send reset link. Please try again later." }));
        setIsLoading(false);
        return;
      }
  ...
  useEffect(() => {
    ...
    const cleanup = () => { authClient.oneTap({
          ...
          <div className="w-full max-w-3xl p-12 relative">
            ...
            >
              ...
              >
                ...
                >
                  <label className="col-span-3 text-sm font-medium text-gray-700">Email <span className="text-red-500">*</span></label>
                ...
                <motion.div 
                  ...
                  variants={itemVariants}
                >
                  <label className="col-span-3 text-sm font-medium text-gray-700">Password <span className="text-red-500">*</span></label>
          ...
          >
            ...
            {!resetSent ? (
              ...
              >
                ...
                >
                  Password reset link sent to <span className="font-medium">{forgotEmail}</span>
                </motion.p>
                <motion.button
                  onClick={() => setShowForgotPassword(false)}
