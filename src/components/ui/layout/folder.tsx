...
const Folder: React.FC<FolderProps> = ({
	...
	// Create file items if files are provided
	const fileItems = files.slice(0, actualMaxItems).map((file, index) => {
		if (file.type === 'folder') {
			return (
				<div key={file.id} className="p-1 text-xs flex items-center space-x-1">
					<span>📁</span>
			...
			return (
				<div key={file.id} className="p-1 text-xs flex items-center space-x-1">
					{showFileIcons && <span style={{ color: fileColor }}>{icon}</span>}
					<span className="truncate">{file.name}</span>
				</div>
			)
		}
	...
	// Use files if provided, otherwise use items
	const displayItems = files.length > 0 ? fileItems : items

	// Slice and pad items array
	const papers = displayItems.slice(0, actualMaxItems)
	while (papers.length < actualMaxItems) {
		papers.push(undefined as any)
	}
	...
	return (
		<div style={scaleStyle} className={cn('', className)}>
			<div
				className={cn(
