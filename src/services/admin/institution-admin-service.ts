...
export const fetchInstitutionDetails = async (
	...
	import("@/types/domain/institution-details").InstitutionDetails | null
> => {
	try {
		const response = await fetch(
			`/api/admin/institutions/${institutionId}`
		);
		const result: import("@/types/domain/institution-details").ApiResponse<
			import("@/types/domain/institution-details").InstitutionDetails
		> = await response.json();
...
export const performInstitutionAction = async (
	institutionId: string,
	actionData: import("@/types/domain/institution-details").InstitutionActionRequest
...
export const fetchInstitutionsList = async (
	filters?: import("@/types/domain/institution-details").InstitutionFilters
): Promise<{
	institutions: import("@/types/domain/institution-details").InstitutionListItem[];
	total: number;
	page: number;
	totalPages: number;
} | null> => {
